...
sub create {
    ...
    my ($address, $domain, $goto) = map { $self->param($_) } qw/ address domain goto/;
    ...
    foreach(@gotos) {
        unless($self->check_email_validity($_)) {
            $self->flash_error($self->i18n('pInvalidMailRegex') . ": $_");
    ...
    my @res = $self->model('alias')->get_by_address($address, $domain)->hashes;
